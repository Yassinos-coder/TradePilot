...
- **Fixed a migration instruction that broke the COT feature** — `supabase/README.md` said the files in `migrations/` were historical and must not be run. That is true of the two pre-v2 files, but `cot_history` exists *only* in `2026-08-09-cot-history.sql` and is absent from the schema file, so following the instruction left the COT tables missing entirely. Each migration is now listed with whether it applies.
